...
Uso:
    1. Crie um arquivo link_drive.txt com uma URL do Google Drive por linha
    2. Execute: python drive.py
    3. Ou especifique outro arquivo: python drive.py meus_link_drive.txt
"""
...
def ler_links(caminho: str) -> list[str]:
    ...
    if not os.path.exists(caminho):
        ...
        print(f"   Crie o arquivo e adicione uma URL do Google Drive por linha.")
        sys.exit(1)
    ...
    with open(caminho, "r", encoding="utf-8") as f:
        for num, linha in enumerate(f, start=1):
            ...
            if "drive.google.com" not in url:
                print(f"⚠️  Linha {num}: URL ignorada (não parece ser do Google Drive): {url}")
                continue
            links.append(url)
...
def main() -> None:
    ...
    print("=" * 55)
    print("     Google Drive Downloader — powered by yt-dlp")
    print("=" * 55)
