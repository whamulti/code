...
Uso:
    1. Crie um arquivo link_youtube.txt com uma URL por linha
    2. Execute: python youtube_downloader.py
    3. Ou especifique outro arquivo: python youtube_downloader.py meus_link_youtube.txt
"""
...
def ler_links(caminho: str) -> list[str]:
    ...
    if not os.path.exists(caminho):
        ...
        print(f"   Crie o arquivo e adicione uma URL do YouTube por linha.")
        sys.exit(1)
    ...
    with open(caminho, "r", encoding="utf-8") as f:
        for num, linha in enumerate(f, start=1):
            ...
            if "youtube.com" not in url and "youtu.be" not in url:
                print(f"⚠️  Linha {num}: URL ignorada (não parece ser YouTube): {url}")
                continue
            links.append(url)
